...
export const getPrismaErrorMessage = (str: string) => {
  ...
  return str
    ? DbForeignList[str.replace(' (index)', '')] || defaultMessage
    : defaultMessage;
};
...
export class GlobalExceptionFilter implements ExceptionFilter {
    ...
    if (
      ...
      exception &&
      exception['code'] &&
      PrismaErrors[exception['code']]
    ) {
      if (exception['code'] == 'P2003' && exception.meta.field_name) {
        //...
      } else if (exception['code'] == 'P4000') {
        response.status(422).json({
          error_code: 'VALIDATION_ERROR',
          message: PrismaErrors[exception['code']],
        });
      } else {
        response.status(400).json({
          error_code: 'GENERAL_ERROR',
          message: PrismaErrors[exception['code']] || 'Genel Hata',
        });
      }
